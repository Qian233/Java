...
//		Populates the recommendedNutrientsList using persons¡¯
//		calculateEnergyRequirement()<return energy> and calculateNutriRequirement() methods.

		//recommendedNutrientsList.add(new RecommendedNutrient("Energy", person.calculateEnergyRequirement()));
		
		//changed
		person.recommendedNutrientsList.clear();
		//need to initialize every time creating a person
		person.initializeNutriConstantsTable();
		person.recommendedNutrientsList.add(new RecommendedNutrient(ENERGY_NUTRIENT_CODE, 
				person.calculateEnergyRequirement()));
		//
		
		for(int i = 0; i < person.calculateNutriRequirement().length; i++) {
			//person.calculateNutriRequirement()[i];//nutriConstants for nutrient i
			for(NutriEnum ne: NutriEnum.values()) {
				if(ne.getNutriIndex() == i) {
					person.recommendedNutrientsList.add(new RecommendedNutrient(ne.getNutrientCode(), 
